...
class Detective extends React.Component {
  ...
  DetectiveList() {
    return this.props.detectiveBooks.map((book) => {
      return (
  ...
  render() {
    return (
      <div>
        ...
        <BooksCollection collectionName='Detectivs' collection={this.DetectiveList()}/>
...
function bookStateToProps(state) {
  return {
    detectiveBooks: state.detectiveBooks,
...
export default connect(bookStateToProps, matchDispatchToProps)(Detective);

Detective.propTypes = {
  detectiveBooks: PropTypes.array.isRequired,
